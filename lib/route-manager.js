...
RouteManager.prototype.installHandlers = function() {
  ...
  this.on('discoverall', function(reqHandler){
    _this.deviceManager.discoverAll(function(err) {
      if (!err) {
  ...
  this.on('stopdiscover', function(reqHandler) {
    _this.deviceManager.stopDiscoverAll(function(err) {
      if (!err) {
        reqHandler.response(200);
      } else {
        reqHandler.response(500, err.message);
  ...
  this.on('disconnect', function(reqHandler) {
    var deviceID = reqHandler.req.params.deviceID;
    var token = reqHandler.req.body.device_token;
    _this.deviceManager.disconnectDevice(deviceID, token, function(err) {
      if (!err) {
        reqHandler.response(200);
      } else {
        reqHandler.response(500, err.message);
  ...
  this.on('getdeviceschema', function(reqHandler) {
    ...
    _this.deviceManager.getDeviceSchema(deviceID, path, token, function(err, data) {
      if (!err) {
        reqHandler.response(200, data);
      } else {
        reqHandler.response(500, err.message);
