...
RouteManager.prototype.mountDevicePresentationPage = function(deviceID) {
  ...
  this.deviceManager.getDeviceRootUrl(deviceID, function(err, deviceUrl) {
    if (!err) {
      var proxyArgs = [];
      try {
        var proxyUrl = url.format(deviceUrl);
        proxyArgs.push(proxyUrl);
      } catch (e) {
        console.error('device root URL malformed: %s', e.message);
        return;
      }

      //TODO: monitor parent crash and kill child
      var parent = cp.fork(__dirname + '/proxy.js', proxyArgs);
      parent.on('message', function(msg) {
        ...
        this.presentationRouter.use('/', function(req, res) {
          // TODO: in real production this must be https instead
          var redirectedUrl = protocol + hostIp + ':' + port + req.url;
