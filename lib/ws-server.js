...
WSServer.prototype.getEventSubscriber = function(ws, options, callback) {
  this.subscriberManager.getSubscriber(ws, options, callback);
};

WSServer.prototype.findEventSubscriber = function(ws, options, callback) {
  this.subscriberManager.findEventSubscriber(ws, options, callback);
};

WSServer.prototype.removeSubscriber = function(subscriber, callback) {
  this.subscriberManager.removeSubscriber(subscriber, callback);
};

WSServer.prototype.findAllEventSubsribers = function(ws, callback) {
  this.subscriberManager.findAllEventSubsribers(ws, callback);
};
...
WSServer.prototype.onSocketClose = function(code, message) {
  this.wsServer.findAllEventSubsribers(this, function(err, subscribers) {
    ...
    for (var s in subscribers) {
      ...
      this.wsServer.eventUnsubscribe(subscriber, opt.deviceID, opt.serviceID, opt.device_access_token, function(err) {
        ...
        this.wsServer.removeSubscriber(subscriber, function(error, subscriber, options) {
          if (error) {
...
WSServer.prototype.onInputMessage = function(message, flags) {
  ...
  switch(inputMessage.topic) {
    case 'subscribe':
      ...
      this.wsServer.getEventSubscriber(this, options, function(subscriber) {
        this.wsServer.eventSubscribe(subscriber, deviceID, serviceID, device_access_token, function(err) {
          if (err) {
            ...
            this.wsServer.removeSubscriber(subscriber, function(){});
          }
        }.bind(this));
      }.bind(this));
      break;
    case 'unsubscribe':
      ...
      this.wsServer.findEventSubscriber(this, options, function(subscriber) {
        if (subscriber === null) {
...
WSServer.prototype.installHandlers = function() {
