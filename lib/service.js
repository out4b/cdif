...
Service.prototype.validate = function(action, arguments, isInput, callback) {
  ...
  if (isInput) {
    for (var i in argList) {
      if (argList[i].direction === 'in') {
        if (arguments[i] == null) {
          failed = true;
          error = new Error('missing input argument: ' + i);
          break;
        }
      }
    }
...
Service.prototype.invokeAction = function(actionName, input, callback) {
  ...
  if (action == null) {
    ...
    this.validate(action, input, true, function(err) {
      if (err) {
        callback(err, null);
      } else {
        action.invoke(input, function(err, output) {
          if (!err) {
            _this.validate(action, output, false, function(err) {
              if (err) {
                callback(err, null);
              } else {
