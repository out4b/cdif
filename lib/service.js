...
Service.prototype.getState = function(callback) {
  var output = {};
...
//return state of a single variable
Service.prototype.getVariableState = function(name, callback) {
  var output = {};
  if (!this.states[name]) {
    callback(new Error('invalid variable name: ' + name), null);
  } else {
    output[name] = this.states[name].value;
    callback(null, output);
  }
};
...
Service.prototype.sendEvent = function(data) {
  ...
  for (var i in data) {
    if (this.states[i] == null) {
      ...
      console.error(this.serviceID + ' sending unknown data: ' + i);
      return;
    }
