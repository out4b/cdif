...
function DeviceManager(mm) {
  ...
  this.moduleManager.on('deviceonline', function(discovered, module) {
    ...
    discovered.on('event', function(serviceId, data) {
...
DeviceManager.prototype.eventSubscribe = function(deviceID, serviceId, callback) {
  var device = deviceMap[deviceID];
  if (!device) {
    callback(new Error('device not found'), null);
    return;
  ...
  if (!deviceObj) {
    ...
    if (device.module.discoverState === 'discovering') {
      ...
      deviceObj.subscribeDeviceEvent(serviceId, function(err) {
        callback(err, null);
      });
    }
  }
