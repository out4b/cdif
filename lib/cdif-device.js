...
function CdifDevice(spec) {
  ...
  this.eventCallback = function(serviceId, data) {
    this.emit('event', serviceId, data);
  }.bind(this);
}
...
CdifDevice.prototype.subscribeDeviceEvent = function(serviceId, callback) {
  ...
  if (this.subscribeEvent) {
    if (this.listeners(serviceId).length === 0) {
      this.addListener(serviceId, function(data) {
        this.eventCallback(serviceId, data);
      });
    }
...
CdifDevice.prototype.unSubscribeDeviceEvent = function(serviceId, callback) {
  ...
  if (this.unsubscribeEvent) {
    this.unsubscribeEvent(serviceId, function(err) {
      if (!err) {
        if (_this.listeners(serviceId).length === 1) {
          _this.removeAllListeners(serviceId);
        }
      }
      callback(err);
    });
