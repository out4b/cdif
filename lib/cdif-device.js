...
CdifDevice.prototype.setOAuthAccessToken = function(params, callback) {
  if (typeof(this._setOAuthAccessToken) === 'function') {
    this._setOAuthAccessToken(params, function(err) {
      if (err) {
        callback(new CdifError(err.message));
      } else {
        this.connected = true;
        this.connections++;
        callback(null);
...
CdifDevice.prototype.setDeviceTimeout = function(callback) {
  if (this.timeout == null) {
    ...
    //TODO: consider oauth redirect flow this might be 30s or longer in production, or handle it explicitly
    setTimeout(this.timeout, 45000);
  }
};
