...
CdifDevice.prototype.connect = function(user, pass, callback) {
  if (this.connected === true) {
    if (this.spec.device.userAuth === true) {
      if (this.secret === '') {
        return callback(new CdifError('cannot verify password'), null);
      }
      if (this.user !== user) {
        return callback(new CdifError('username not match'), null);
      }
      this.auth.compareSecret(pass, this.secret, function(err, res) {
        ...
        if (!err) {
          return callback(new CdifError('password not match'), null);
        }
  ...
  if (this._connect && typeof(this._connect) === 'function') {
    this._connect(user, pass, function(error) {
      if (error) {
        return callback(new DeviceError('connect fail: ' + error.message), null);
      }
...
CdifDevice.prototype.installTimeoutHandlers = function(eventName, callback) {
  // do not allow parallel interface access
  if (this.listeners(eventName).length === 0) {
    this.setDeviceTimeout(eventName, callback, function(cb) {
      this.online = false;
      this.connected = false;
      this.connections = 0;
      this.removeAllListeners(eventName);
      this.removeAllListeners('authredirect');
...
CdifDevice.prototype.setDeviceTimeout = function(eventName, cb, callback) {
  ...
  setTimeout(timeout.expire, 40000);
};
...
CdifDevice.prototype.removeEventListener = function(eventName) {
  this.removeAllListeners(eventName);
};
